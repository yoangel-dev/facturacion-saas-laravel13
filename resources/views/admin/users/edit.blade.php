...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <div class="border-b border-slate-800 pb-4">
        <h1 class="text-2xl font-black text-white">Editar Usuario: {{ $user->name }}</h1>
        <p class="text-xs text-slate-400 mt-1">Modifica la asignación de tenant o contraseña.</p>
    </div>

    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="rounded-2xl border border-slate-800 bg-slate-950 p-6 shadow-sm space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase text-slate-300 mb-1">Nueva Contraseña (Dejar en blanco para mantener)</label>
            ...
            <div>
                ...
                <select name="role" class="w-full rounded-xl bg-slate-900 border-slate-800 text-sm text-white focus:border-amber-500 focus:ring-amber-500/20">
                    <option value="admin" {{ $user->role === 'admin' ? 'selected' : '' }}>Admin (Administrador de Tenant)</option>
                    <option value="superadmin" {{ $user->role === 'superadmin' ? 'selected' : '' }}>Superadmin (Acceso Global)</option>
                    <option value="user" {{ $user->role === 'user' ? 'selected' : '' }}>User (Usuario Estándar)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="tenant_id" class="w-full rounded-xl bg-slate-900 border-slate-800 text-sm text-white focus:border-amber-500 focus:ring-amber-500/20">
                    <option value="">-- Sin Tenant (Requerido para Superadmin) --</option>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            ...
            <button type="submit" class="px-6 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-md transition-all">Actualizar Usuario</button>
        </div>
    </form>
